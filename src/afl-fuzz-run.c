...
common_fuzz_stuff(afl_state_t *afl, u8 *out_buf, u32 len) {
  ...
  len = write_to_testcase(afl, out_buf, len, 0); // Write modified data to file for testing

  fault = fuzz_run_target(afl, &afl->fsrv, afl->fsrv.exec_tmout); // actually running it
  ...
  afl->queued_discovered += save_if_interesting(afl, out_buf, len, fault); // checks if the exec is interesting, if yes : updates the bitmap and returns 1 
